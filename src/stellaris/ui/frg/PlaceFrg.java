...
public class PlaceFrg extends PlacementFragment{
    final int rowWidth = 4;

    public AsCat currentCategory = AsCat.distribution;

    Seq<Block> returnArray = new Seq<>(), returnArray2 = new Seq<>();
    Seq<AsCat> returnCatArray = new Seq<>();
    boolean[] categoryEmpty = new boolean[AsCat.all.length];
    ObjectMap<AsCat,Block> selectedBlocks = new ObjectMap<>();
    ObjectFloatMap<AsCat> scrollPositions = new ObjectFloatMap<>();
    ...
    boolean gridUpdate(InputHandler input){
        ...
        if(Core.input.keyDown(Binding.pick) && player.isBuilder()){ //mouse eyedropper select
            ...
            if(tryRecipe != null && tryRecipe.isVisible() && unlocked(tryRecipe)){
                ...
                currentCategory = AsCat.toAs(input.block.category);
                return true;
            }
        }
        ...
        for(int i = 0; i < blockSelect.length; i++){
            if(Core.input.keyTap(blockSelect[i])){
                if(i > 9) { //select block directionally
                    ...
                    if(!getUnlockedByCategory(AsCat.all[i]).isEmpty()){
                        currentCategory = AsCat.all[i];
                        if(input.block != null){
    ...
    Seq<AsCat> getCategories(){
        return returnCatArray.clear().addAll(AsCat.all).sort((c1, c2) -> Boolean.compare(categoryEmpty[c1.ordinal()], categoryEmpty[c2.ordinal()]));
    }

    Seq<Block> getByCategory(AsCat cat){
        return returnArray.selectFrom(content.blocks(), block -> AsCat.toAs(block.category) == cat && block.isVisible());
    }

    Seq<Block> getUnlockedByCategory(AsCat cat){
        return returnArray2.selectFrom(content.blocks(), block -> AsCat.toAs(block.category) == cat && block.isVisible() && unlocked(block)).sort((b1, b2) -> Boolean.compare(!b1.isPlaceable(), !b2.isPlaceable()));
    }

    Block getSelectedBlock(AsCat cat){

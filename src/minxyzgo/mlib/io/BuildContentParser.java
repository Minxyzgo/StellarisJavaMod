...
public class BuildContentParser{
    ...
    public void readConsumes(Consumers consumes, String owner, String key, JsonValue value) {
        for(JsonValue child : value.get(key)){
            if(child.name.equals("item")){
                consumes.item(find(ContentType.item, child.asString()));
            }else if(child.name.equals("items")){
                consumes.add((Consume)parser.readValue(ConsumeItems.class, child));
            }else if(child.name.equals("liquid")){
                consumes.add((Consume)parser.readValue(ConsumeLiquid.class, child));
            }else if(child.name.equals("power")){
                if(child.isNumber()){
                    consumes.power(child.asFloat());
                }else{
                    consumes.add((Consume)parser.readValue(ConsumePower.class, child));
                }
            }else if(child.name.equals("powerBuffered")){
                consumes.powerBuffered(child.asFloat());
            }else{
                throw new IllegalArgumentException("Unknown consumption type: '" + child.name + "' for block '" + owner + "'.");
            }
        }
    }
    ...
    private Prov<Unit> unitType(JsonValue value){
        ...
        } else if(EntityMapping.nameMap.containsKey(type)) {
            return (Prov<Unit>)EntityMapping.map(type);
        } else {
